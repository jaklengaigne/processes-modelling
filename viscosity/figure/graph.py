...
class Graph:
    ...
    def viscosity_as_a_function_of_temperature_and_shear_stress(self, deck, polymer, model):
        ...
        # The higher the discretisation number is, the finer the discretisation
        # will be, the higher the number of points in the graphic.
        ...
        for k in range(len(T)):
            ...
            for i in range(discretisation):
                melt_viscosity = model.viscosity(polymer.constantB,
                                                 polymer.energy,
                                                 R, T[k],
                                                 polymer.constantb,
                                                 shear_stress_power[i])
                log_melt_viscosity.append(log10(melt_viscosity))
            log_melt_viscosity = numpy.array(log_melt_viscosity)
            axes.plot(shear_stress_power, log_melt_viscosity, color[k],
                      label=legend[k])
        ...
        axes.legend()
        plt.savefig("./graphics/viscosity_as_a_function_of_temperature_and_shear_stress.pdf",
                    format="pdf")
