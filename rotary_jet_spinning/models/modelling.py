...
class RJSModel:
    ...
    def __init__(self, polymer, machine):
        self.omega_th = self.critical_rotational_velocity_threshold(
            polymer.surface_tension, machine.orifice_radius,
            machine.reservoir_radius, polymer.density)

        self.initial_velocity = self.Initial_velocity(self.omega_th,
                                                      machine.reservoir_radius)

        self.nu = self.kinematic_viscosity(polymer.viscosity, polymer.density)

        self.final_radius(machine.orifice_radius, self.initial_velocity,
                          self.nu, machine.collector_radius, machine.omega)
    ...
    def radius(self, r0, rho, initial_velocity, x_position, mu, Sigma, omega):
        ...
        - *Sigma* (float) - call sigma function : sigma(surface_tension,
                                                        x_position, r0,
                                                        initial_velocity)
